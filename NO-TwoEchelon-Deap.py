...
X_weighted = df.loc[:, ['Latitude', 'Longitude', 'Assign1']]
...
def AsgnFunc(ind):
    ...
    AsgnDCFact = np.zeros(shape=(TDCN, FactN))
    for dc in ind:
        for fact in range(FactN):
...
def Fact_DC_Cust_Prod_Time(ind):
    FDPCT = np.zeros(shape=(FactN, TDCN, CustN, ProdN, Timehorizon))
    FDPCT_DC_cust_Tr = np.zeros(shape=(FactN, TDCN, CustN, ProdN, Timehorizon))
    FDPCT_fact_DC_Tr = np.zeros(shape=(FactN, TDCN, CustN, ProdN, Timehorizon))
    InitialInv_Tr = np.zeros(shape=(FactN, TDCN, ProdN, Timehorizon))
    for fact in range(FactN):
        for dc in range(TDCN):
            if ind[0][dc][fact] == 1:
                for c in range(CustN):
...
def evalcost(ind):
    ...
    HC = DCTotalInv_cost(ind).sum() * ch  # Total holding costs
